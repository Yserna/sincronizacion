...
void* counter(int numThreads);

/* Global variables */
int MAXCNT, NUMTHREADS, APPROXIMATION_FACTOR;
counter_t count;
...
int main(int argc, char *argv[]) { 
    ...
    /* Initializing conter */
    init(&count, APPROXIMATION_FACTOR);
    ...
    for (int i = 0; i < NUMTHREADS; i++){
        /* Thread creation */
        pthread_create(&threadsArray[i], NULL, &counter, i);
    }
